...
pub enum PropertyId {
    ...
    /// releases.
    PipelineHandler = PIPELINE_HANDLER,
...
impl PropertyId {
    ...
    pub fn description(&self) -> &'static str {
        match self {
            PropertyId::Location => "Camera mounting location
",
            PropertyId::Rotation => {
                "The camera physical mounting rotation. It is expressed as the angular
difference in degrees between two reference systems, one relative to the
...
/// releases.
#[derive(Debug, Clone)]
pub struct PipelineHandler(pub String);
impl Deref for PipelineHandler {
    type Target = String;
    fn deref(&self) -> &Self::Target {
        &self.0
    }
}
impl DerefMut for PipelineHandler {
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.0
    }
}
impl TryFrom<ControlValue> for PipelineHandler {
    type Error = ControlValueError;
    fn try_from(value: ControlValue) -> Result<Self, Self::Error> {
        Ok(Self(<String>::try_from(value)?))
    }
}
impl From<PipelineHandler> for ControlValue {
    fn from(val: PipelineHandler) -> Self {
        ControlValue::from(val.0)
    }
}
impl ControlEntry for PipelineHandler {
    const ID: u32 = PropertyId::PipelineHandler as _;
}
impl Property for PipelineHandler {}
/// The arrangement of color filters on sensor; represents the colors in the
/// top-left 2x2 section of the sensor, in reading order. Currently
/// identical to ANDROID_SENSOR_INFO_COLOR_FILTER_ARRANGEMENT.
#[cfg(feature = "vendor_draft")]
...
pub fn make_dyn(
    ...
    match id {
        ...
        PropertyId::PipelineHandler => Ok(Box::new(PipelineHandler::try_from(val)?)),
        #[cfg(feature = "vendor_draft")]
        PropertyId::ColorFilterArrangement => {
